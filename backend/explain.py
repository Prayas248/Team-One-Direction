...
if GROQ_API_KEY:
    ...
    GROQ_STATUS = "✓ Groq API configured"
    print(GROQ_STATUS)
else:
    client = None
    print("⚠️  GROQ_API_KEY not found in .env")
    print("⚠️  Using fallback explanations only")
...
def explain_flag(flag: Dict) -> str:
    ...
    if flag.get('layer') == 3:
        ...
        feature = flag.get('feature', 'unknown metric')
        
        # Ensure section name is descriptive
        section_display = f"'{section}'" if section and section != 'Unknown' else 'a section of'
        
        # Determine deviation severity
        severity = 'moderate' if abs(z_score) < 3 else 'substantial'
        
        return (
            f"The text shows {severity} style deviation (z-score: {z_score:.2f}) in {feature}, "
            f"suggesting possible authorship inconsistency that warrants editorial review."
        )
...
Task: Explain in 1-2 sentences WHY these passages are similar. Focus on shared ideas, phrasing patterns, or structural similarities. Use editorial language appropriate for academic publishing."""
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="llama-3.3-70b-versatile",  # Groq's latest open model
            messages=[
                {
                    "role": "system",
